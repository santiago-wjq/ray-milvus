...
class ChunkReader:
    ...
    def get_chunks(self, indices: Union[List[int], np.ndarray], parallelism: int = 1) -> List["pa.RecordBatch"]:
        """
        ...
        for i in range(num_arrays[0]):
            ...
            array_ptr = self._ffi.cast(
                "struct ArrowArray*",
                int(self._ffi.cast("uintptr_t", arrays_ptr)) + i * self._ffi.sizeof("struct ArrowArray"),
            )
    ...
    def get_chunk_indices(self, row_indices: Union[List[int], np.ndarray]) -> np.ndarray:
        ...
        chunk_indices = np.frombuffer(
            self._ffi.buffer(chunk_indices_ptr[0], num_chunks[0] * self._ffi.sizeof("int64_t")), dtype=np.int64
        ).copy()
...
class Reader:
    ...
    def __init__(
        ...
        if not isinstance(schema, pa.Schema):
            raise InvalidArgumentError(f"schema must be a pyarrow.Schema, got {type(schema).__name__}")
        ...
        result = self._lib.reader_new(
            column_groups.encode("utf-8"), c_schema, columns_array, num_columns, self._props._get_c_properties(), handle
        )
        check_result(result)
    ...
    def take(self, indices: Union[List[int], np.ndarray], parallelism: int = 1) -> "pa.RecordBatch":
        ...
        result = self._lib.take(
            self._handle, self._ffi.cast("int64_t*", indices_ptr), len(indices_array), parallelism, c_array
        )
        check_result(result)
    ...
    def get_chunk_reader(self, column_group_id: int) -> ChunkReader:
        ...
        if column_group_id < 0:
            raise InvalidArgumentError(f"column_group_id must be non-negative, got {column_group_id}")
