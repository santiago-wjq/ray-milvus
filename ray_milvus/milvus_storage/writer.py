...
class Writer:
    ...
    def __init__(self, path: str, schema: "pa.Schema", properties: Optional[Dict[str, str]] = None):
        ...
        result = self._lib.writer_new(path.encode("utf-8"), c_schema, self._props._get_c_properties(), handle)
        check_result(result)
    ...
    def write(self, batch: "pa.RecordBatch") -> None:
        ...
        if not isinstance(batch, pa.RecordBatch):
            raise InvalidArgumentError(f"batch must be a pyarrow.RecordBatch, got {type(batch).__name__}")
